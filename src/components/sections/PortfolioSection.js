...
export default function PortfolioSection({ projectsData }) {
  return (
    ...
    >
      ...
      {/* fifth project */}
      <div
        className={`bg-[#471d5f] pb-10 pt-10 flex flex-col-reverse justify-between items-start md:grid md:grid-cols-2 md:pb-0 gap-20 md:pt-52`}
      >
        <div
          className={`hidden bg-[url('/assets/projects/zuzu.png')] bg-right-top object-contain bg-no-repeat shadow-[36px_-20px_66px_-4px_rgba(0,0,0,0.37)] w-full h-[420px] rounded-tr-[40px] md:flex`}
        ></div>

        <div className=" flex flex-col space-y-5 px-6 mt-24 md:mt-0">
          <div className="text-center md:text-left">
            <h1 className="text-white text-2xl font-bold uppercase">
              ZUZU Music Dashboard
            </h1>
          </div>
          <p className="text-white text-justify">
            It&apos;s a music dashboard implementation from a UI/UX Design
          </p>
          <div
            className={`flex bg-[url('/assets/projects/zuzu.png')] bg-contain bg-no-repeat  w-full h-[200px] rounded-xl md:hidden`}
          ></div>
          ...
          <div className="flex flex-col justify-center space-y-5 md:space-y-0 md:justify-start md:flex-row md:space-x-10">
            <Button
              onClick={() => window.open("https://zuzu-music.netlify.app/")}
              Icon={VisibilityIcon}
              text="View Live"
              bordered
            />
            <Button
              onClick={() =>
                window.open("https://github.com/obendesmond/musicapp-dashboard")
              }
              Icon={GitHubIcon}
              text="View Code"
            />
